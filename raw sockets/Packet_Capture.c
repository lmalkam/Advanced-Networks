...
void my_callback(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    if (stop_capture)
    {
       ...
       // pcap_breakloop((pcap_t *)args);  // Break out of pcap_loop
        return;
    }
    ...
    {
        printf("UDP\n");
        const char *message = (const char *)(packet + sizeof(struct ether_header) + sizeof(struct ip) +
        ...
        {
            printf("Found the message 'Linesh'. Stopping capture.\n");
            stop_capture = 1;
        }
    }
    return;
