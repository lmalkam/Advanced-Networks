...
unsigned short chksum(unsigned short *b, int len) 
{    
...
int main() 
{
    ...
    // saddr.sin_addr.s_addr = inet_addr("111.123.113.134");
	...
	iph->version = 4;
	iph->tos = 0;
	iph->tot_len = sizeof(struct iphdr) + sizeof(struct tcphdr) + strlen(message);
	iph->id = htonl(54321);
	iph->frag_off = 0;
	iph->ttl = 255;
    ...
    iph->daddr = inet_addr("111.123.113.134");

    //tcp header
    tcph->source = htons(51721);
	tcph->dest = htons(6969);
	tcph->seq = 0;
	tcph->ack_seq = 0;
	tcph->doff = 0;
	tcph->fin = 0;
