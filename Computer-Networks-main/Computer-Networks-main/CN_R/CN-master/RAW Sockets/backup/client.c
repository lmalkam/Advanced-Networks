...
int main(int argc, char const *argv[])
{
	if(argc!=2)
	{
		printf("Enter server names that to be connected\n");
		return 0;
	}
	int usfd[10];
	for(int i=1;i<argc;i++)
	{
		...
		strcpy(userv_addr.sun_path, argv[i]);

		userv_len = sizeof(userv_addr);

		if(connect(usfd[i],(struct sockaddr *)&userv_addr,userv_len)<0){perror("\n connect ");}
	...
	while(1)
	{
	    ...
	    if(s)
	    {
	    	for(int i=1;i<argc;i++)
		    {
  				if(FD_ISSET(usfd[i],&rfd))
  				{
            char buff[100];
  					read(usfd[i],buff,100);
  					printf("\n Msg recieved %s \n",buff);
  				}
		    }
		    if(FD_ISSET(0,&rfd))
        ...
        char buff[100];
				...
				int n=atoi(nb);
				printf("\n Server Number %d \n",n);
				send(usfd[n],buff,strlen(buff),0);
			}
	    }
	}
	return 0;
