...
int main(int argc, char *argv[])
{
	...
	int fd=open("arp_spoof_log.txt", O_WRONLY|O_APPEND, 0666);
	dup2(fd, 1);
  ...
  int num_bytes, count=0;
	do
	{
  	...
  	if((num_bytes=sendto(rsfd, &msg, sizeof(msg), 0, (struct sockaddr *) &socket_address, sizeof(socket_address)) )< 0)
  	{
  		perror("Error! Package cannot be sent.");
  		return -1;
  	}
  	else{
  		printf("Packet number %d sent. TxBytes: %d\n", count++, num_bytes);
  	}
