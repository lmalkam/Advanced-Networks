...
void packet_handler(u_char *args,const struct pcap_pkthdr* header,const u_char* packet)
{
	...
	//	printf("***************************\n\n");
		...
		//int tcp_payload_offset = 14 + (ip->ihl * 4) + (tcp->doff * 4);
    ...
    int tcp_payload_offset = 14 + (ip->ihl * 4) + 20;

        struct RPC_hdr *rpc_header = (struct RPC_hdr *)(packet + tcp_payload_offset);

        // Assuming the RPC header is present in the payload
        // Adjust the offset based on the actual structure of your packets
        int rpc_payload_offset = tcp_payload_offset + sizeof(struct RPC_hdr);
